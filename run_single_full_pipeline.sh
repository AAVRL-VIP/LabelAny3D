...
# One-shot single-image pipeline:
# 1) Auto segmentation (EntityV2/CLIPSeg/OVSAM, fallback to rembg)
# 2) Build COCO-style coconut_val.json for the single image
# 3) Run README pipeline steps end-to-end
#
# Usage:
#   bash run_single_full_pipeline.sh /abs/path/image.jpg
# Optional env:
#   GPU_IDX=0 OBJ_REC=trellis MIN_MASK_AREA=6400

if [[ $# -lt 1 ]]; then
  echo "Usage: bash run_single_full_pipeline.sh /abs/path/image.jpg"
  exit 1
fi

IMAGE_PATH="$1"
GPU_IDX="${GPU_IDX:-0}"
OBJ_REC="${OBJ_REC:-trellis}"
...
# Try EntityV2 + CLIPSeg + OVSAM first
try:
...
# Fallback: rembg + connected-component split for multi-object scenes
if len(masks) == 0:
...
# Ensure label count matches mask count.
...
# Save segmentation visualization artifacts for inspection.
...
if len(annotations) == 0:
    raise RuntimeError(
        "No valid segmentation instances were produced. "
        "Try a different image or reduce MIN_MASK_AREA."
    )
PY
fi

echo "[2/7] Depth estimation..."
